...
const extractHeaderAndReleases = (tokens, rawContent) => {
	const header     = { tokens: [], raw: rawContent.shift(), content: [] };
	const unreleased = {};

	let detailingUnreleased = false;

	const releases = tokens.reduce((list, token) => {
		const { type, depth, text } = token;
		...
		if (type === 'heading' && depth === 2) {
			...
			if (text !== UNRELEASED && text !== UNRELEASED_TITLE) {
				list.push({
					tokens: { title: token, details: [] },
					raw: { title: text, details: rawDetails }
				});
				detailingUnreleased = false;

			} else {
				unreleased.tokens = { title: token, details: [] };
				unreleased.raw = { title: text, details: rawDetails };
				detailingUnreleased = true;
			}

		// Unreleased details
		} else if (detailingUnreleased) {
			unreleased.tokens.details.push(token);

		// Release details
		} else if (list.length !== 0) {
			list[list.length - 1].tokens.details.push(token);
	...
	return { header, unreleased, releases };
};
...
const processRelease = (release, links) => {
	...
	release.raw.description = rawDetails.shift();
	...
	if (release.raw.title === UNRELEASED_TITLE) {
		...
		const { version, versionFull, date, flag, flagFull } = capturingGroupValues(release.raw.title, RELEASE_PATTERN);
	...
	release.changes = release.tokens.details.reduce((list, token) => {
		...
		if (type === 'heading' && depth === 3) {
			...
			release.tokens.changes.push({ type: token, content: [] });
			release.raw.changes.push({ type: text, content: rawChanges });
...
export default (content) => {
	...
	let { header, unreleased, releases } = extractHeaderAndReleases(tokens, rawContent);  // eslint-disable-line prefer-const
	...
	return { tokens, header, unreleased, releases };
};
